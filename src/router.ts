...
export default class Router {
  ...
  async navigate(path: string) {
    ...
    const [matchedRoute, remainingPath] = this.matchRoute(path.split('/').filter(Boolean));
    ...
    if (matchedRoute) {
      ...
      if (remainingPath.length > 0 && matchedRoute.children) {
        this.navigateNested(componentInstance, matchedRoute.children, remainingPath);
      }
    }
  }

  async navigateNested(parentComponent: any, children: { [key: string]: Route }, pathSegments: string[]) {
